...
filename <- "C:/Users/Lenovo/Dropbox/Investigacion_2019/Curso_mineria_texto/Ejercicio_Ultimo.rdata"
load(filename)
...
text1 <- "Transformación universidad."
...
grepl(pattern = "universidad", x = text1)
grepl("universidad", text1)
...
# en oraciones
strsplit(x = text, split = ".", fixed = T) # separación 

## modificar mayusculas/minusculas
#todos los caracteres minusculas
tolower(text) # minúsculas
...
tablita <-grepl(pattern = "internet",
     x = articles.flt.dt$title.pr,
     perl = T)  # se busca la palabra internet

table(tablita)
tabli2 <-grepl("( |^)universidad(es)?( |$)",articles.flt.dt$title.pr ,
      perl = T)

tablita <-grepl(pattern = "mercadeo",
                x = articles.flt.dt$title.pr,
                perl = T)
table(tabli2)

tablita <-grepl(pattern = "branch",
                x = articles.flt.dt$title,
                perl = T)
table(tablita)


###
articles.flt.dt[, text.pr := str_conv(text, "UTF-8")]
articles.flt.dt[, text.pr := tolower(text.pr)]
View(articles.flt.dt[, list(text, text.pr)])

# Tablita que quiere El "Renato"

tablita <-grepl(pattern = "branch",
                x = articles.flt.dt$text.pr,
                perl = T)
table(tablita)


gsub(pattern = "alberto marín morán [0-9]+ views [0-9]+ comentarios",
     replacement = "",
     articles.flt.dt$text.pr,
     perl = T)
...
dtm_f10<-dfm_trim(dtm_w2, min_termfreq = 5, min_docfreq = 3)
dtm_f10
...
simtop <- sim[freqfeatures,]
library(GGally)
ggcorr(data = NULL, cor_matrix = simtop, nbreaks=10, palette='RdBu', label=TRUE, label_size=4, 
...
textplot_wordcloud(dtm_sw, min_count = 20, random_order = FALSE,
                   ...
                   color = RColorBrewer::brewer.pal(20,"Dark2"))

textplot_wordcloud(dtm_sw, min_count = 20, random_order = FALSE,
                   rotation = .25, min_size = 1, max_size = 5,
                   color = c("red", "black", "cyan", "yellow", "green", "brown", "orange", "violet"))
